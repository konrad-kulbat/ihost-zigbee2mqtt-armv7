...
ENV_PROBE=z2m-env-test; MQTT=mqtt-test; MOCK=mock-supervisor; SUB=z2m-mqtt-sub
...
expect_probe_env() {
  grep -Fx -- "$2=$3" "$1/.ci-node-environ" >/dev/null || fail "environment passed to node index.js lacks $2=$3"
}
diagnostics() {
  printf '\n===== diagnostics =====\n' >&2; docker ps -a >&2 || true
  for c in "$Z2M" "$ENV_PROBE" "$SUPERVISOR_Z2M" "$SOCAT_Z2M" "$MQTT" "$MOCK"; do
  ...
  for c in "$ENV_PROBE" "$SUPERVISOR_Z2M"; do docker container inspect "$c" >/dev/null 2>&1 && node_environment "$c" >&2 || true; done
}
cleanup() {
  status=$?; [ "$status" -eq 0 ] || diagnostics
  docker rm -f "$Z2M" "$ENV_PROBE" "$SUPERVISOR_Z2M" "$SOCAT_Z2M" "$MQTT" "$MOCK" "$SUB" >/dev/null 2>&1 || true
...
for c in "$Z2M" "$ENV_PROBE" "$SUPERVISOR_Z2M" "$SOCAT_Z2M" "$MQTT" "$MOCK" "$SUB"; do docker container inspect "$c" >/dev/null 2>&1 && fail "refusing to replace existing container: $c"; done
...
mkdir -p "$DATA_DIR" "$WORK_DIR/socat-data" "$WORK_DIR/supervisor-data"
printf '%s\n' '{"data_path":"/config/zigbee2mqtt","socat":{"enabled":false},"mqtt":{"server":"mqtt://mqtt-test:1883","base_topic":"zigbee2mqtt"},"serial":{}}' > "$WORK_DIR/options.json"
printf '%s\n' '{"data_path":"/config/zigbee2mqtt","socat":{"enabled":false},"mqtt":{},"serial":{}}' > "$WORK_DIR/supervisor-options.json"
printf '%s\n' '{"data_path":"/config/zigbee2mqtt","socat":{"enabled":true,"master":"PTY,raw,echo=0,link=/tmp/ttyZ2M,mode=777","slave":"TCP-LISTEN:8485,reuseaddr,fork","options":"-d -d","log":true},"mqtt":{"server":"mqtt://mqtt-test:1883"},"serial":{}}' > "$WORK_DIR/socat-options.json"
printf '%s\n' 'setInterval(() => {}, 1000);' > "$WORK_DIR/env-probe.js"
printf '%s\n' '#!/bin/sh' 'tr "\000" "\n" </proc/self/environ > /config/zigbee2mqtt/.ci-node-environ' 'PATH=/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin' 'exec "$(command -v node)" "$@"' > "$WORK_DIR/node"
chmod 0755 "$WORK_DIR/node"
...
docker run -d --name "$MOCK" --network "$NETWORK" --network-alias mock-supervisor --user root --entrypoint node --health-cmd 'node -e "fetch(\"http://127.0.0.1/services/mqtt\", {headers:{Authorization:\"Bearer test-token\"}}).then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"' --health-interval 1s --health-timeout 3s --health-retries 30 node:22-alpine -e 'const http=require("http");const body=JSON.stringify({data:{host:"mqtt-test",port:1883,ssl:false,username:"test-user",password:"test-password"}});http.createServer((req,res)=>{if(req.url!=="/services/mqtt"||req.headers.authorization!=="Bearer test-token"){res.writeHead(401);return res.end();}res.writeHead(200,{"content-type":"application/json"});res.end(body);}).listen(80,"0.0.0.0")' >/dev/null
docker run --rm --network "$NETWORK" eclipse-mosquitto:2 sh -ec 'until nc -z mqtt-test 1883; do sleep 1; done'
for _ in {1..30}; do [ "$(docker inspect -f '{{.State.Health.Status}}' "$MOCK")" = healthy ] && break; sleep 1; done
[ "$(docker inspect -f '{{.State.Health.Status}}' "$MOCK")" = healthy ] || fail 'mock Supervisor healthcheck did not become healthy'
SUPERVISOR_RESPONSE=''
for _ in {1..30}; do
...
docker exec --user root "$Z2M" sh -ec 'touch /config/zigbee2mqtt/ci-test-persistence'; test -f "$DATA_DIR/ci-test-persistence"; docker rm -f "$Z2M" >/dev/null
test -f "$DATA_DIR/ci-test-persistence"
docker run -d --name "$Z2M" --platform linux/arm/v7 --network "$NETWORK" -p 18099:8099 "$(readonly_mount "$WORK_DIR/options.json" /data/options.json)" "$(readwrite_mount "$DATA_DIR" /config/zigbee2mqtt)" "$IMAGE" >/dev/null
wait_http; pass 'persistence'; pass 'restart'
docker run -d --name "$SUPERVISOR_Z2M" --platform linux/arm/v7 --network "$NETWORK" -e SUPERVISOR=http://mock-supervisor:80 -e SUPERVISOR_TOKEN=test-token "$(readonly_mount "$WORK_DIR/supervisor-options.json" /data/options.json)" "$(readonly_mount "$WORK_DIR/env-probe.js" /app/index.js)" "$(readonly_mount "$WORK_DIR/node" /usr/local/sbin/node)" "$(readwrite_mount "$WORK_DIR/supervisor-data" /config/zigbee2mqtt)" "$IMAGE" >/dev/null
for _ in {1..30}; do [ -s "$WORK_DIR/supervisor-data/.ci-node-environ" ] && break; sleep 1; done
expect_probe_env "$WORK_DIR/supervisor-data" ZIGBEE2MQTT_CONFIG_MQTT_SERVER mqtt://mqtt-test:1883; expect_probe_env "$WORK_DIR/supervisor-data" ZIGBEE2MQTT_CONFIG_MQTT_USER test-user; expect_probe_env "$WORK_DIR/supervisor-data" ZIGBEE2MQTT_CONFIG_MQTT_PASSWORD test-password; pass 'Supervisor MQTT discovery'
...
for _ in {1..30}; do docker exec --user root "$SOCAT_Z2M" sh -ec 'ps | grep -q [s]ocat' && break; sleep 1; done
docker exec --user root "$SOCAT_Z2M" sh -ec 'ps | grep -q [s]ocat'; docker inspect -f '{{.State.Running}}' "$SOCAT_Z2M" | grep -Fx true >/dev/null; pass 'socat smoke test'
